...
def setup_support_data(method="tsne", drop_labels=True):
    ...
    X.drop(columns=["death", "sfdm2"], inplace=True)
...
def test_between_all_3():
    ...
    fig = plt.figure(constrained_layout=True, figsize=((7.125-0.17), ((7.125-0.17)/1.8)/1.618), dpi=1000, facecolor="w",edgecolor="k",)
    ...
    spec23 = gridspec.GridSpecFromSubplotSpec(2, 2, subplot_spec=spec2[1], hspace=0.01)# wspace=-0.5, hspace=-0.9)
    ...
    scs = []
    for val, i in zip([0, 1], [0, 2]):
        sc = axi[0].scatter(standard_embedding[labels == val,0], standard_embedding[labels == val,1], marker= '.', color=matplotlib.colormaps["Accent"].colors[i], alpha=0.2)
        scs.append(sc)
        axi[1].scatter(standard_embedding[labels == val,0], standard_embedding[labels == val,1], marker= '.', color=matplotlib.colormaps["Accent"].colors[i], alpha=0.2)
        axi[2].scatter(standard_embedding[labels == val,0], standard_embedding[labels == val,1], marker= '.', color=matplotlib.colormaps["Accent"].colors[i], alpha=0.2)
    ...
    for (i, o), axi in zip(enumerate(arrow_labels), [ax21, ax22, ax23, ax24]):
        im = axi.scatter(
    ...
    # cbar = fig.colorbar(im, ax=[ax21, ax22, ax23, ax24], pad=0.02, aspect=40)
